/**
 * Encrypted Token Storage for X (Twitter) Credentials
 * 
...
class XTokenStorage {
  private readonly STORAGE_KEY = 'screndly_x_tokens';
  ...
  async initialize(): Promise<void> {
    ...
    this.encryptionKey = await crypto.subtle.deriveKey(
      {
        name: 'PBKDF2',
        salt: new TextEncoder().encode('screndly-x-salt-v1'),
        iterations: 100000,
        hash: 'SHA-256',
      },
      keyMaterial,
  ...
  private async getKeyMaterial(): Promise<CryptoKey> {
    const password = process.env.ENCRYPTION_SECRET || 'screndly-x-default-secret-change-in-production';
...
export const xTokenStorage = new XTokenStorage();
